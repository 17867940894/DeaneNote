...
![在这里插入图片描述](javaEE/assets/20210621210829596.png)
...
![在这里插入图片描述](javaEE/assets/2021062121084476.png)
...
![在这里插入图片描述](javaEE/assets/20210621210906576.png)
...
![在这里插入图片描述](javaEE/assets/20210621210919531.png)
...
![在这里插入图片描述](javaEE/assets/20210621210931769.png)
...
![在这里插入图片描述](javaEE/assets/20210621212202445.png)
...
![在这里插入图片描述](javaEE/assets/20210621212220521.png)

> 在线咨询： **两个人都可以是发送方，也可以是接收方(配合[[11-多线程]])**

![在这里插入图片描述](javaEE/assets/20210621212230346.png)
        ...
        while (true) {
            try {
                ...
                DatagramPacket packet = new DatagramPacket(
                        dataBytes, //发送的容器
                        0,//开始的下标
                        dataBytes.length,//结束的下标
                        new InetSocketAddress(this.toIP,this.toPort));//从哪里开始到哪里去
                // 发送执行
                socket.send(packet);
                if (data.equals("bye")) {
                    break;
...
public class TalkReceive implements Runnable{
    ...
    public void run() {
        while (true) {
            ...
            byte[] data = packet.getData();
            String receiveData = new String(data,0,packet.getLength(),StandardCharsets.UTF_8);
...
import uitls.TalkReceive;
import uitls.TalkSend;
...
![image-20240614144458506](javaEE/assets/image-20240614144458506.png)

> 如果出现互相沟通中出现乱码:例如?????
>
> 则尝试以下设置
>
> ![image-20240614144721272](javaEE/assets/image-20240614144721272.png)
...
![在这里插入图片描述](javaEE/assets/20210621212303606.png)
...
![在这里插入图片描述](javaEE/assets/20210621212333948.png)
...
> **getBy Name(String host)**    
>
> ​	获取与Host相对应的InetAddress对象
>
> **getHostAddress()**               
>
> ​	 获取InetAdress对象所包含的IP地址
>
> **getHostName()**                   
>
> ​	 获取此IP地址的主机名
...
## 10.InetSocketAddress 常用方法

此类用于实现 IP 套接字地址 (IP 地址+端口号)，用于socket 通信

==InetSocketAddress== 类常用的方法

- InetAddress getAddress() 	//获取 InetAddress 对象
- int getPort() 	//获取端口号
- String getHostName() 	//获取主机名

## 11.Java网络编程之DatagramSocket类

​    使用DatagramSocket类的实例对象可以发送和接收DatagramPacket数据包。在创建发送端和接收端的DatagramSocket对象时，使用的构造方法也有所不同，下面对DatagramSocket类中常用的构造方法进行讲解。

### 　　(1)DatagramSocket()

　　该构造方法用于创建发送端的DatagramSocket对象，在创建DatagramSocket对象时，并没有指定端口号，此时，系统会分配一个没有被其他网络程序使用的端口号。
...
　　使用该构造方法在创建DatagramSocket对象时，不仅指定了端口号，而且指定了相关的IP地址。该构造方法适用于计算机上有多块网卡的情况，在使用时可以明确规定数据通过哪块网卡向外发送或接收哪块网卡的数据。由于计算机中会为不同的网卡分配不同的IP，所以在创建DatagramSocket对象时需要通过指定IP地址确定使用哪块网卡进行通信。

　　上面讲解了DatagramSocket的常用构造方法，下面对DatagramSocket类的常用方法进行介绍，DatagramSocket类的常用方法如表9-3所示。

　　表9-3 DatagramSocket类的常用方法



![1700789863270_datagramSocket类的常用方法.jpg](javaEE/assets/786f94576fa4d705c83a589f4c0cf8ba.jpeg)

　　表9-3中，对DatagramSocket类中的常用方法及其功能进行了介绍。其中，send()方法用于发送DatagramPacket数据包，receive()方法用于将接收到的数据填充到DatagramPacket数据包中，close()方法用于关闭当前的Socket。

## 12.URL类的常用方法

一个URL对象生成之后其属性是不能被改变的,但是可以通过它给定的方法来获得这些属性
常用方法:

```
public String getProtocol();//获得该URL的协议名
public String getHost();//获取该URL的主机名/域名(一个域名之下可以有多个主机名)
...
注意: URL结构中从端口号之后的文件名 + 片段名 + 参数列表都算作这里的文件名输出
