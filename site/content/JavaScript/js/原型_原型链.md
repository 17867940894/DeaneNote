...
也就是说 **[[数组]]是对象、函数是对象、正则是对象、对象还是对象。**

<img src="./assets/1460000042725372.png" alt="img" style="zoom:200%;" />

## 二、原型和原型链是什么

上面我们说到对象就是属性（property）的集合，有人可能要问不是还有方法吗？其实方法也是一种属性，因为它也是`键值对`的表现形式，具体见下图。

<img src="./assets/1460000042725373.png" alt="img" style="zoom:200%;" />
...
![img](JavaScript/js/assets/1460000042725374.png)
...
![img](JavaScript/js/assets/1460000042725375.png)
...
![img](JavaScript/js/assets/1460000042725376.png)
...
3. 参考视频讲解：[进入学习](https://link.segmentfault.com/?enc=jHekQl1nehlrGVP2WnKAGg%3D%3D.lS11ax4uvCfG%2FkHZr9KoCC1wkOW6eWRB%2FQABFLSD70fR6%2BGjgGxICndCdnsGTs1S)
...
![img](JavaScript/js/assets/1460000042725377.png)
...
![img](JavaScript/js/assets/1460000042725378.png)
...
> instanceof运算符用于测试构造函数的[[prototype]]属性是否出现在对象的原型链中的任何位置
...
> ## 1. 用户认证与权限控制
>
> - JWT认证 ：用户登录后获取token，请求时携带token进行身份验证
> - 角色管理 ：区分管理员(admin)和普通用户
> - 路由守卫 ：保护需要登录的页面，未登录自动跳转登录页
>
> ## 2. 对局管理逻辑
>
> - 对局组创建 ：创建包含多场比赛的对局组, 任何人都可以新建对局, 但是只有参赛人员和管理员可以修改对局中的比分, 被确认过的比分只有管理员可以进行修改. 任何的对局信息和排行榜所有人都可以查看
> - 对局生成算法 ：
>   - 单打：生成所有选手两两组合的对局, 每名选手连续对局数量不能超过2次
>   - 双打：生成所有无重复成员的2人队伍组合
>
> ## 3. 比分记录逻辑
>
> - 实时比分更新 ：每次页面的数据在新增,修改,删除时需要刷新页面的数据
> - 快捷分数按钮 ：11、15、20、21分快速设置
> - 胜负判定 ：达到设定分数(11/15/21)判定胜负, 在确认比分后才能显示选手之间的胜负
> - 状态管理 ：待开始 , 进行中 , 已完成, 
>
> ## 4. 权限控制逻辑
>
...
> ## 6. 数据持久化
>
> - 对局数据存储 ：对局组、场次、选手信息存储到[[16-数据库]]
> - 事务处理 ：创建对局组时使用数据库事务保证数据一致性
> - 状态同步 ：前端状态与后端数据实时同步
>
> ## 7. 错误处理与状态回滚
>
> - 异常捕获 ：API调用失败时捕获错误并提示用户
> - 状态回滚 ：服务器保存失败时前端状态回滚到可编辑状态
>
> ## 8. 业务规则验证
>
> - 选手选择限制 ：只能从已注册用户中选择参赛选手
> - 比赛规则配置 ：支持11分制、15分制、21分制
> - 对局轮数配置 ：支持1-5轮对局
