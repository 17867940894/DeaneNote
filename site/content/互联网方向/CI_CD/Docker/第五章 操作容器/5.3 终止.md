...
```mermaid
flowchart TD
    cmd["docker stop mycontainer"]
    A["1. 发送 SIGTERM 信号给容器主进程（PID 1）"]
    B["2. 等待容器优雅退出（默认 10 秒）"]
    C["3. 如果超时仍未退出，发送 SIGKILL 强制终止"]
...
| 命令 | 说明 |
|---|---|
| `docker stop <容器>` | 优雅停止容器（推荐） |
| `docker stop -t <秒> <容器>` | 自定义超时时间停止 |
| `docker kill <容器>` | 强制停止容器 |
| `docker kill -s <信号> <容器>` | 发送自定义信号 |
