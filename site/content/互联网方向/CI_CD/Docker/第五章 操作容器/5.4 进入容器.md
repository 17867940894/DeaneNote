...
flowchart LR
    ...
    Attach -->|附加到这里| Container["容器"]
    Container --> P1["PID 1: /bin/bash<br>（你的输入直接发送到主进程）"]
```
...
```mermaid
flowchart TB
    subgraph docker_attach["docker attach"]
        A_Main["附加到主进程"] --> A_PID1["PID 1: bash"]
        A_PID1 --> A_Note["退出 bash 容器停止"]
    end

    subgraph docker_exec["docker exec"]
        E_New["新进程"] --> E_PID1["PID 1: nginx"]
        E_New --> E_PID50["PID 50: bash"]
        E_PID50 --> E_Note["退出 bash 不影响 nginx"]
    end
```
...
| 命令 | 说明 |
|---|---|
| `docker exec -it <容器> bash` | 进入容器执行交互式 shell（推荐） |
| `docker exec <容器> <命令>` | 在容器中执行单条命令 |
| `docker exec -u root <容器> <命令>` | 以 root 用户执行命令 |
| `docker attach <容器>` | 附加到容器的主进程 |
| `Ctrl+P Ctrl+Q` | 从 attach 会话中安全退出（不停止容器） |
| `docker debug <容器>` | 使用调试工具箱进入容器（Docker Desktop Pro+） |
