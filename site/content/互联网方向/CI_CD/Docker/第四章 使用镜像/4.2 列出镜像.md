...
> `ubuntu:24.04` 是具体版本号，`ubuntu:noble` 是发布代号（Ubuntu 24.04 的代号）。在 Dockerfile 中应优先使用版本号（如 `ubuntu:24.04`）而非发布代号。


## 三、理解镜像大小

### 3.1 本地大小 vs Hub 显示大小
...
`docker image ls` 默认列出的只是顶层镜像。还有一种镜像是为了加速镜像构建、重复利用资源而存在的中间层镜像。
...
| 命令 | 说明 |
|---|---|
| `docker image ls` | 列出镜像（标准格式） |
| `docker images` | 列出镜像（简写） |
| `docker images -q` | 只显示镜像 ID |
| `docker images -a` | 包含中间层镜像 |
| `docker images --digests` | 显示摘要 |
| `docker images --no-trunc` | 显示完整 ID |
