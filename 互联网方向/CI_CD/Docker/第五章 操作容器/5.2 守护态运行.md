...
> **这是初学者最常遇到的困惑。** 理解这个问题，就理解了 Docker 的核心设计理念。
...
然后用 `docker ps` 查看，发现容器根本不在运行！

### 3.2 核心原理：容器的生命周期与主进程绑定
...
> **版本说明**：生产环境建议明确指定具体版本号（如 `mysql:8.4.4`）而非使用 `latest`。
...
| `docker container ls -a` | 查看所有容器（含已停止） |
| `docker logs <容器>` | 查看容器日志 |
| `docker logs -f <容器>` | 实时跟踪日志 |
| `docker logs --tail 100 <容器>` | 查看最后 100 行日志 |
| `docker port <容器>` | 查看端口映射 |
| `docker attach <容器>` | 连接到后台容器的终端 |


## 延伸阅读

- [进入容器](https://yeasy.github.io/docker_practice/05_container/5.4_attach_exec.html)：如何进入正在运行的容器执行命令
- [HEALTHCHECK 健康检查](https://yeasy.github.io/docker_practice/07_dockerfile/7.12_healthcheck.html)：自动检测容器内服务是否正常
- [Docker Compose](https://yeasy.github.io/docker_practice/11_compose/index.html)：管理多个后台容器的更好方式
