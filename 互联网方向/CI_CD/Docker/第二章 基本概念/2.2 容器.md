...
|---|---|
| **进程空间** | 容器看不到宿主机上的其他进程 |
| **网络** | 默认拥有独立的网络命名空间，可分配独立 IP |
| **文件系统** | 容器拥有独立的 root 目录 |
| **用户** | 容器内的 `root` 通常只拥有受限 capabilities |
...
flowchart TB
    subgraph 容器方案
        direction TB
        CApp["应用层<br>App A / App B"]
        CContainer["隔离层<br>Container 仅应用 / Container 仅应用"]
        CE["Docker Engine"]
        CH["Host OS"]
        CHW["Hardware"]
    ...
    subgraph 虚拟机方案
        direction TB
        VMApp["应用层<br>App A / App B"]
        VMGuest["Guest OS<br>完整系统 / 完整系统"]
        V["Hypervisor"]
        VMH["Host OS"]
        VMHW["Hardware"]
...
| **启动时间** | 通常更快（秒级） | 通常更慢（分钟级） |
| **资源占用** | 通常更低（MB 级别） | 通常更高（GB 级别） |
| **运行开销** | 通常更接近原生 | 通常有更高虚拟化开销（5-20%） |
| **内核** | 共享宿主机内核 | 各自独立内核 |


## 四、容器的存储层

理解容器的存储层机制对于数据的持久化和镜像的优化至关重要。
...
> **这是新手最容易踩的坑！容器存储层与容器生命周期绑定。容器删除，数据就没了！**
